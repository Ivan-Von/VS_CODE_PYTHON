...
def build_formula(formula_str):
    ...
    if formula_str[0] == "!":
        ...
        flag = 0
    #分割得到原子公式的各个部分
    formula_list = re.split(r'(?:[(),])', formula_str)
...
def get_condition(s):
    ...
    for clause_str in open(filePath,encoding = 'utf-8'):
        #print(clause_str,end='')
        #clause_str = input() #输入一个条件子句
        #clause_str = clause_str.replace(' ','').strip()
        clause_formulas = [] #用于储存一个子句中的原子公式
        #如果以"("开头说明该子句中有多个原子公式
        #clause_str = str(clause_str)
        #print(clause_str,end='')
        if(clause_str[0] == "("):
...
def get_new_formulas(formula1, formula2, clause1_c, clause2_c):
    ...
    # if len(new_formulas) == 2:
    #     if new_formulas[0] == new_formulas[1]:
    #         new_formulas.pop()
    return new_formulas

#对两个子句进行归结合一
def guijieheyi(clause1, clause2, s):
...
def matching(s, start_clause):
    ...
    for i in range(n):
        ...
        for j in range(max(i + 1, start_clause), n):
            clause2 = s[j]
            #对两个子句进行归结合一
            end = guijieheyi(clause1, clause2, s)
            #如果归结结束
            if end == 1:
                return 1
    return 0
